...
口径说明：以上百分比是"用地分类口径"＝各代码在 geometry/land_use.geojson 中的概念地块面积之和 ÷ 场地面积（按 EPSG:4548 面积计算，7 类合计约 100%，即约 1141 公顷，与场地面积同口径互检）；本表为全包唯一用地分类，道路与水系未单列占用比例。metrics.json 的 green_ratio（约 11.0%）是"绿线概念绿地率口径"＝geometry/green_space.geojson 面积 ÷ 场地面积，两者分层不同、互不替代；官方用地与边界数据发布后按同一聚合规则整体复算，此前不以 unknown 或占位值代替。
