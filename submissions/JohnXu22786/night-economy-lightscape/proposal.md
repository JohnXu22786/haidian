...
对标案例只借机制、不搬尺度，逐案公开来源登记如下（细节不作引用、不编造；"首个""率先"等表述均为公开渠道口径，未做独立事实核查，必要时按待研究假设处理）：
...
| 1401 | 公园绿地 | 约 22.7% | 低（概念粗绘） |
| 0901 | 商业用地 | 约 15.5% | 低（概念粗绘） |
| 0701 | 城镇住宅用地 | 约 11.0% | 低（概念粗绘） |
| 0902 | 商务金融用地 | 约 9.1% | 低（概念粗绘） |
| 0803 | 文化用地 | 约 8.1% | 低（概念粗绘） |
| 0903 | 娱乐用地 | 约 2.2% | 低（概念粗绘） |
| 其他 | 道路、水域与暂未分区概念地块 | 约 4.6% | 低（概念粗绘） |

口径说明：以上百分比是"用地分类口径"＝各代码在 geometry/land_use.geojson 中的概念地块面积之和 ÷ 场地面积（合计约 95.4%，即约 1088 公顷）；metrics.json 的 green_ratio（约 11.0%）是"绿线概念绿地率口径"＝geometry/green_space.geojson 面积 ÷ 场地面积，两者分层不同、互不替代，全包只维护这一组分类与两种已解释口径；官方用地与边界数据发布后按同一聚合规则整体复算，此前不以 unknown 或占位值代替。
...
> **证据锚点**：[source:DATA-SRC-MNR-LAND-USE-CLASSIFICATION]（用地分类名称参照现行国标口径）；[metric:green_ratio]（绿地率口径区分）。
...
| 对标案例 | 六个案例与四项机制对比 | 公开渠道概述并逐案登记，不引用数据细节 |
