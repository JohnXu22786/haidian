---
title: "NIGHT·JZ Night Light Loop: Night Economy & Lightscape Operation Belt (Concept)"
...
![Regional Cooperation Interfaces (concept)](assets/figures/regional-cooperation.en.png)
...
![AI Innovation Ecosystem Benchmarks & Eight-Element Mechanism (concept)](assets/figures/ai-ecosystem.en.png)
...
![Three-Node Index & Detailed Concept Design](assets/figures/key-areas.en.png)
...
![Functional Night Nodes x Pilgrimage Landmarks - Honors - Component Library (concept)](assets/figures/landmarks-night-nodes.en.png)
...
Caliber statement: the percentages above are the "land-use-class caliber" = sum of concept parcel areas per code in geometry/land_use.geojson divided by site area (computed in EPSG:4548; the 7 classes total approx. 100%, about 1,141 ha, cross-checked against the site area); this table is the package's only land-use classification, and roads and water bodies are not separately itemised. The green_ratio in metrics.json (approx. 11.0%) is the "concept greenline caliber" = geometry/green_space.geojson area divided by site area. The two calibers are distinct and not interchangeable; when official land-use and boundary data are released, everything is recomputed by the same aggregation rule, and unknown or placeholder values are not used in the meantime.
...
> **Evidence anchors**: [source:DATA-SRC-MNR-LAND-USE-CLASSIFICATION] (land-use class names follow the current national standard); [metric:green_ratio] (greenline-caliber distinction).
...
![Slow Mobility & Blue-Green Analysis (lighting tiers & dark-sky friendly zones)](assets/figures/mobility-bluegreen.en.png)
...
![Core Metrics Evidence Chart (rounded display)](assets/figures/metrics-evidence.en.png)
